...
const Nav = () => {
  ...
  useEffect(() => {
    const handleOutsideClick = (ref, setShow) => (e) => {
      if (!ref.current.contains(e.target)) {
        setShow(false);
      }
    };

    const clickHandler1 = handleOutsideClick(showRef, setShow);
    const clickHandler2 = handleOutsideClick(showAppRef, setShowApp);
    const clickHandler3 = handleOutsideClick(showSettingRef, setShowSetting);

    document.addEventListener("mousedown", clickHandler1);
    document.addEventListener("mousedown", clickHandler2);
    document.addEventListener("mousedown", clickHandler3);

    return () => {
      document.removeEventListener("mousedown", clickHandler1);
      document.removeEventListener("mousedown", clickHandler2);
      document.removeEventListener("mousedown", clickHandler3);
    };
  }, [
    showRef,
    setShow,
  ...
  return (
    <>
      <div className="flex justify-between items-center px-2 pt-2 pr-4">
        ...
        <div className="w-[40%] md:w-[50%] lg:w-[60%] xl:w-[70%] 2xl:w-[70%] 3xl:w-[70%] flex justify-end mx:justify-between lg:justify-between xl:justify-between 2xl:justify-between 3xl:justify-between items-center">
          <div
            className={`${
              showInput
                ? "w-[170px] px-2 md:px-0 duration-500"
                : "w-[50px] duration-500"
          ...
          >
            ...
            <input
              ...
              className={`outline-none bg-secondary-300 ${
                showInput
                  ? "w-[110px] duration-500"
                  : "w-0 md:w-[90%] duration-500"
              }`}
            />
          </div>
          <div className="flex justify-center items-center rounded-full space-x-1 md:space-x-2">
            <div className="hidden md:block lg:block xl:block 2xl:block 3xl:block">
              <div className="relative group flex justify-center items-center rounded-full  w-[40px] h-[40px] cursor-pointer hover:bg-secondary-200">
                <svg
                  width="20"
                  height="20"
                  fill="#686b70"
                ...
                <span className="hidden z-20 group-hover:block absolute top-12 -left-8 w-[100px] p-2 bg-secondary-500 text-white font-bold rounded scale-[70%]">
                  <p>Help menu</p>
                </span>
              </div>
            </div>
            ...
            >
              ...
              <ChangePassword ref={showSettingRef} showSetting={showSetting} />
              <span className="hidden z-10 group-hover:block absolute top-12 -left-10 w-[125px] p-2 bg-secondary-500 text-white font-bold rounded scale-[70%]">
                <p>Settings menu</p>
              </span>
            </div>
          </div>
        </div>
        <div className="w-[20%] md:w-[7%] lg:w-[7%] xl:w-[7%] 2xl:w-[7%] 3xl:w-[7%] flex items-center space-x-1 md:space-x-2">
          <div
            ref={showAppRef}
